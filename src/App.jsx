...
@media print{
  ...
  function openVehicleDeliveryForm(operationItem) {
    ...
    if (!vehicle?.id) return setError('Selecciona primero un vehículo.')
    const sourceReservation = reservationsStore.items.find((row) => String(row.id || row.__docId || row.docId || '') === String(operationItem?.reservationId || operationItem?.docId || operationItem?.__docId || '')) || {}
