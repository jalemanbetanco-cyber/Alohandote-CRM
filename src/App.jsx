...
async function buildAlohandoteCleanPdfBlob(){
  ...
  if(page){
  var isMobileContract=/Android|iPhone|iPad|iPod/i.test(navigator.userAgent)||window.innerWidth<768;
  if(isMobileContract){
    ...
    try{
      ...
      if(mobileSignatures){mobileSignatures.style.marginTop='28px';mobileSignatures.style.gap='32px';}
  ...
  async function syncAllExternalIcalsSilent() {
    ...
    try {
      ...
      const accommodationsWithIcal = accommodations.filter((apt) => (
        apt?.id && accommodationIcalUrls(apt).length && icalSchedulerIsDue(apt, now)
      ))
