...
function emptyReservation(vehicleId, selectedDate = '', profile = null, user = null) {
  return {
    ...
    deliveryKm: '', kmRecepcion: '', kmRecorridos: '', dailyKmLimit: DAILY_KM_LIMIT, includedKm: '', additionalKmPurchased: '', authorizedKm: '', excessKm: '', receptionAt: '', receivedBy: '', fuelLevelReception: '', receptionStatus: '', licenseDoc: null, idDoc: null, paymentMethod: 'Pago en BS', paymentReference: '', paymentProof: null, totalAmountBs: '', amountBs: '', bcvEuroRate: '', maintenanceLaborCost: '', maintenancePartsCost: '', maintenanceLaborCostBs: '', maintenancePartsCostBs: '', maintenanceUsdEquivalent: '', maintenancePaymentMethod: 'BS', expenseStatus: 'Pagado', maintenanceBsCost: '', bcvDollarRate: '', maintenanceInvoices: [],
...
function maintenanceEuroRate(item = {}, exchangeRates = null) {
  return euroRateValue(exchangeRates, item.bcvEuroRate || item.bcvDollarRate || item.bcvUsdRate || '')
}
function maintenanceUsdCost(item = {}, exchangeRates = null) {
  // V223.5.19: misma lógica de Registrar movimiento.
  // En Bs, el valor digitado se conserva como Bs real y el USD se calcula
  // con la tasa EURO BCV congelada. En divisas, el costo se guarda en USD.
  const bucket = maintenancePaymentBucket(item.maintenancePaymentMethod || '')
  const rate = maintenanceEuroRate(item, exchangeRates)
  const directUsd = num(item.maintenanceUsdEquivalent || item.maintenanceCostUsd || 0)
  const componentUsd = num(item.maintenanceLaborCost) + num(item.maintenancePartsCost)
  const directBs = num(item.maintenanceBsCost || item.maintenanceCostBs || 0)
  const componentBs = num(item.maintenanceLaborCostBs) + num(item.maintenancePartsCostBs)
  if (bucket === 'Bs') {
    const bs = componentBs > 0 ? componentBs : directBs
    if (bs > 0 && rate > 0) return Number((bs / rate).toFixed(2))
    if (directUsd > 0) return Number(directUsd.toFixed(2))
    return Number(num(item.maintenanceCost || 0).toFixed(2))
  }
  const usd = componentUsd > 0 ? componentUsd : (directUsd > 0 ? directUsd : num(item.maintenanceCost || 0))
...
function maintenanceBsCost(item = {}, exchangeRates = null) {
  ...
  if (bucket === 'Bs') {
    const bs = componentBs > 0 ? componentBs : directBs
    if (bs > 0) return Number(bs.toFixed(2))
  }
  const rate = maintenanceEuroRate(item, exchangeRates)
  return Number((maintenanceUsdCost(item, exchangeRates) * rate).toFixed(2))
}
function maintenanceMovementPaymentMethod(method = '') {
  const bucket = maintenancePaymentBucket(method)
  if (bucket === 'Bs') return 'Pago en BS'
  if (bucket === 'Binance') return 'Usdt'
  if (bucket === 'Zelle') return 'Zelle'
  if (bucket === 'Efectivo $') return '$ Efectivo'
  return method || 'Pago en BS'
}
...
async function buildAlohandoteCleanPdfBlob(){
  ...
  await addImageIfExists('/firma-abogado.png',16,14,30,16);
  ...
  paragraphs.forEach(function(p){
    addText(p.textContent,10.2,false);
  });
...
export default function App() {
  ...
  const vehicleProfitabilityRows = useMemo(() => vehicles.map((vehicle) => {
    ...
    const maintenance = reservationsStore.items
      .filter((reservation) => reservation.vehicleId === vehicle.id && isMaintenanceRecord(reservation) && !isIcalImportedBlock(reservation))
      .reduce((sum, reservation) => sum + maintenanceUsdCost(reservation), 0)
  ...
  const lodgingProfitabilityRows = useMemo(() => accommodations.map((apt) => {
    ...
    const maintenance = (lodgingStore.items || [])
      .filter((reservation) => reservation.accommodationId === apt.id && isMaintenanceRecord(reservation) && !isIcalImportedBlock(reservation))
      .reduce((sum, reservation) => sum + maintenanceUsdCost(reservation), 0)
  ...
  function assetExpensesForDetail(assetType, assetId) {
    ...
    const generalRows = (generalExpensesStore.items || [])
      .filter((expense) => {
      ...
      .map((expense) => ({
        ...expense,
        amountUsd: generalExpenseUsdAmount(expense, exchangeRates),
        amountBsValue: generalExpenseBsAmount(expense, exchangeRates),
      }))

    // V223.5.18: el mantenimiento es un gasto real del activo aunque su fuente
    // permanezca en la reserva/bloqueo de mantenimiento. Se expone aquí como
    // movimiento de gasto para el dashboard del vehículo o alojamiento.
    const mirroredMaintenanceIds = new Set((generalExpensesStore.items || [])
      .filter((row) => row.sourceType === 'maintenanceExpense')
      .map((row) => String(row.sourceReservationId || '')))
    const maintenanceSource = isVehicle ? (reservationsStore.items || []) : (lodgingStore.items || [])
    const maintenanceRowsForAsset = maintenanceSource
      .filter((record) => {
        if (!isMaintenanceRecord(record) || isIcalImportedBlock(record)) return false
        if (mirroredMaintenanceIds.has(String(record.id || record.__docId || ''))) return false
        const recordAssetId = isVehicle ? record.vehicleId : record.accommodationId
        return String(recordAssetId || '') === String(assetId || '') && maintenanceUsdCost(record, exchangeRates) > 0
      })
      .map((record) => {
        const method = record.maintenancePaymentMethod || 'BS'
        const bucket = maintenancePaymentBucket(method)
        const amountUsdValue = maintenanceUsdCost(record, exchangeRates)
        const amountBsValue = maintenanceBsCost(record, exchangeRates)
        const invoices = Array.isArray(record.maintenanceInvoices) ? record.maintenanceInvoices : []
        return {
          id: `asset-maintenance-${isVehicle ? 'vehicle' : 'lodging'}-${record.id}`,
          date: String(record.startDate || record.createdAt || record.updatedAt || new Date().toISOString()).slice(0,10),
          category: 'Mantenimiento',
          description: `${record.maintenanceType || 'Mantenimiento'}${record.note ? ` · ${record.note}` : ''}`,
          paymentMethod: method,
          method,
          currency: bucket === 'Bs' ? 'Bs' : 'USD',
          amount: amountUsdValue,
          amountUsd: amountUsdValue,
          amountBs: amountBsValue,
          amountBsValue,
          expenseStatus: record.expenseStatus || 'Pagado',
          sourceType: 'maintenance',
          sourceId: record.id,
          assetType: isVehicle ? 'Vehículo' : 'Alojamiento',
          assetId: isVehicle ? record.vehicleId : record.accommodationId,
          invoiceFile: invoices[0] || null,
        }
      })

    return [...maintenanceRowsForAsset, ...generalRows]
    ...
    const vehicleRows = reservationsStore.items
      ...
      .flatMap((item) => {
        ...
        const base = {
          ...
          logisticsNote: item.note || item.notes || '',
  ...
  const administrationRows = useMemo(() => {
    ...
    const mirroredMaintenanceKeys = new Set((generalExpensesStore.items || [])
      .filter((row) => row.sourceType === 'maintenanceExpense')
      .map((row) => `${row.sourceModule || ''}|${row.sourceReservationId || ''}`))
    const maintenanceCostRows = maintenanceRowsSafe(reservationsStore.items, lodgingStore.items, vehicles, accommodations)
      .filter((item) => maintenanceUsdCost(item, exchangeRates) > 0)
      .filter((item) => !mirroredMaintenanceKeys.has(`${item.module || ''}|${item.id || ''}`))
      .map((item) => ({
        ...
        amount: maintenanceUsdCost(item, exchangeRates),
        ...
        sourceId: item.id,
        sourceType: 'maintenance',
        assetType: item.module === 'Renta Car' ? 'Vehículo' : 'Alojamiento',
        assetId: item.vehicleId || item.accommodationId || '',
      }))
  ...
  function openPayableSource(row = {}) {
    ...
    if (row.module === 'Alojamientos' || row.module === 'Alojamiento') {
      ...
      if (lodging) return openEditLodging(lodging)
    }
  ...
  const maintenanceRows = useMemo(() => {
    const vehicleMaint = reservationsStore.items
      ...
      .map((item) => {
        const vehicle = vehicles.find((v) => v.id === item.vehicleId) || {}
        const serviceKm = num(item.currentKm || item.deliveryKm || 0)
        const currentKm = num(vehicle.currentKm || serviceKm || 0)
        const nextEveryKm = num(item.nextMaintenanceEveryKm || item.nextMaintenanceKm || 0)
        // V223.5.24: el objetivo se congela cuando se registra el servicio.
        // Nunca se recalcula sumando el intervalo al kilometraje actual del vehículo.
        const targetKm = num(item.maintenanceTargetKm || 0) || (serviceKm > 0 && nextEveryKm > 0 ? serviceKm + nextEveryKm : 0)
        return { ...item, module: 'Renta Car', assetName: vehicle.name || 'Vehículo', serviceKm, currentKm, targetKm, remainingKm: targetKm ? targetKm - currentKm : null }
      })
  ...
  const upcomingMaintenanceRows = useMemo(() =>
    maintenanceRows
      .filter((row) => row.module === 'Renta Car' && row.targetKm > 0 && row.remainingKm !== null && row.remainingKm <= 200)
      .sort((a, b) => num(a.remainingKm) - num(b.remainingKm)),
  [maintenanceRows])

  const selectedVehicleUpcomingMaintenances = useMemo(() => {
    if (!selectedVehicle?.id) return []
    const latestByType = new Map()

    maintenanceRows
      .filter((row) =>
        row.module === 'Renta Car' &&
        String(row.vehicleId || '') === String(selectedVehicle.id) &&
        num(row.targetKm) > 0 &&
        row.remainingKm !== null
      )
      .forEach((row) => {
        const typeKey = String(row.maintenanceType || row.note || row.notes || row.customerName || 'mantenimiento').trim().toLowerCase()
        const current = latestByType.get(typeKey)
        const rowServiceKm = num(row.serviceKm || row.currentKm || row.deliveryKm)
        const currentServiceKm = num(current?.serviceKm || current?.currentKm || current?.deliveryKm)
        const rowDate = String(row.createdAt || row.updatedAt || row.startDate || '')
        const currentDate = String(current?.createdAt || current?.updatedAt || current?.startDate || '')
        if (!current || rowServiceKm > currentServiceKm || (rowServiceKm === currentServiceKm && rowDate > currentDate)) {
          latestByType.set(typeKey, row)
        }
      })

    return [...latestByType.values()]
      .filter((row) => num(row.remainingKm) <= 200)
      .sort((a, b) => num(a.remainingKm) - num(b.remainingKm))
  }, [maintenanceRows, selectedVehicle?.id])
  ...
  const accommodationDashboard = useMemo(() => {
    ...
    return rows.reduce((acc, row) => {
      ...
      } else if (status === 'maintenance' || isMaintenanceRecord(row)) {
        acc.maintenance += 1
  ...
  function openGlobalSearchResult(row) {
    ...
    if (row.module === 'lodging') {
      ...
      } else if (source.id || source.__docId || source.docId || source.reservationId) {
        openEditLodging({ ...emptyLodgingReservation(accommodationId, source.startDate || '', profile, user), ...source })
      }
    } else {
  ...
  async function cleanupLinkedReservationArtifacts(reservation = {}, moduleName = 'Renta Car') {
    ...
    if (!reservationId) return { expenses: 0, tasks: 0, inventoryMovements: 0, operations: 0, publicSubmissions: 0, leads: 0 }

    const isMaintenance = normalizeStatus(reservation.status) === 'maintenance'
    const assetId = String(moduleName === 'Renta Car' ? reservation.vehicleId || '' : reservation.accommodationId || '')
    const maintenanceDate = String(reservation.startDate || reservation.createdAt || '').slice(0,10)
    const maintenanceUsd = maintenanceUsdCost(reservation, exchangeRates)
    const maintenanceBs = maintenanceBsCost(reservation, exchangeRates)
    const maintenanceType = String(reservation.maintenanceType || 'Mantenimiento').toLowerCase()
    const assetName = String(
      moduleName === 'Renta Car'
        ? vehicles.find((item) => String(item.id || '') === assetId)?.name || ''
        : accommodations.find((item) => String(item.id || '') === assetId)?.name || ''
    ).toLowerCase()

    const exactLink = (row = {}) =>
      String(row.sourceReservationId || row.sourceMaintenanceId || row.reservationId || row.sourceId || '') === reservationId

    const sameMaintenanceSignature = (row = {}) => {
      if (!isMaintenance) return false
      if (String(row.category || '').toLowerCase() !== 'mantenimiento') return false
      if (assetId && String(row.assetId || '') !== assetId) return false

      const rowDate = String(row.date || row.startDate || row.createdAt || '').slice(0,10)
      if (maintenanceDate && rowDate && rowDate !== maintenanceDate) return false

      const rowUsd = generalExpenseUsdAmount(row, exchangeRates)
      const rowBs = generalExpenseBsAmount(row, exchangeRates)
      const usdMatches = maintenanceUsd > 0 && Math.abs(rowUsd - maintenanceUsd) <= 0.02
      const bsMatches = maintenanceBs > 0 && Math.abs(rowBs - maintenanceBs) <= 0.02
      if (!usdMatches && !bsMatches) return false

      const description = String(row.description || row.notes || '').toLowerCase()
      const typeMatches = maintenanceType && description.includes(maintenanceType)
      const assetMatches = assetName && description.includes(assetName)
      return typeMatches || assetMatches || !description
    }

    // 1. Eliminar todos los egresos creados por el mantenimiento.
    // La caja se calcula desde estos movimientos: al quitarlos, recupera el saldo.
    let expenses = 0
    const expenseIds = new Set()
    for (const row of (generalExpensesStore.items || []).filter((item) => exactLink(item) || sameMaintenanceSignature(item))) {
      if (!row?.id || expenseIds.has(row.id)) continue
      expenseIds.add(row.id)
      await generalExpensesStore.removeItem(row.id)
      expenses += 1
    }

    // Consulta directa a Firebase para evitar depender del retraso de onSnapshot.
    if (isFirebaseReady && db) {
      const queryFields = [
        ['sourceReservationId', reservationId],
        ['sourceMaintenanceId', reservationId],
        ['sourceId', reservationId],
      ]
      for (const [field, value] of queryFields) {
        try {
          const snapshot = await getDocs(query(collection(db, 'generalExpenses'), where(field, '==', value)))
          for (const found of snapshot.docs) {
            if (expenseIds.has(found.id)) continue
            expenseIds.add(found.id)
            await deleteDoc(doc(db, 'generalExpenses', found.id))
            expenses += 1
          }
        } catch (error) {
          console.warn(`No se pudo limpiar generalExpenses por ${field}:`, error)
        }
      }
    }

    // 2. Eliminar tareas creadas por el mantenimiento.
    let tasks = 0
    const taskIds = new Set()
    const linkedTasks = (hrTasksStore.items || []).filter((task) => {
      if (exactLink(task)) return true
      if (!isMaintenance || !assetId) return false
      const sameAsset = String(task.assetId || '') === assetId
      const sameDate = !maintenanceDate || String(task.dueDate || task.date || '').slice(0,10) === maintenanceDate
      const looksMaintenance = String(`${task.module || ''} ${task.title || ''} ${task.notes || ''}`).toLowerCase().includes('manten')
      return sameAsset && sameDate && looksMaintenance
    })
    for (const task of linkedTasks) {
      if (!task?.id || taskIds.has(task.id)) continue
      taskIds.add(task.id)
      await hrTasksStore.removeItem(task.id)
      tasks += 1
    }

    if (isFirebaseReady && db) {
      for (const field of ['reservationId', 'sourceReservationId', 'sourceMaintenanceId', 'sourceId']) {
        try {
          const snapshot = await getDocs(query(collection(db, 'hrTasks'), where(field, '==', reservationId)))
          for (const found of snapshot.docs) {
            if (taskIds.has(found.id)) continue
            taskIds.add(found.id)
            await deleteDoc(doc(db, 'hrTasks', found.id))
            tasks += 1
          }
        } catch (error) {
          console.warn(`No se pudo limpiar hrTasks por ${field}:`, error)
        }
      }
    }

    // 3. Revertir inventario y eliminar movimientos vinculados.
    let inventoryMovements = 0
    const movementIds = new Set()
    const linkedMovements = (inventoryMovementsStore.items || []).filter((movement) =>
      exactLink(movement) ||
      String(movement.reference || '').includes(reservationId)
    )
    for (const movement of linkedMovements) {
      if (!movement?.id || movementIds.has(movement.id)) continue
      movementIds.add(movement.id)
      const inventoryItem = (inventoryItemsStore.items || []).find((item) => String(item.id || '') === String(movement.itemId || ''))
      if (inventoryItem?.id && String(movement.kind || '').toLowerCase() === 'salida') {
        await inventoryItemsStore.editItem(inventoryItem.id, {
          ...inventoryItem,
          quantity: num(inventoryItem.quantity) + num(movement.quantity),
          lastMovementAt: new Date().toISOString(),
          updatedBy: 'Reversión por eliminación de mantenimiento',
        })
      }
      await inventoryMovementsStore.removeItem(movement.id)
      inventoryMovements += 1
    }

    // 4. Eliminar operaciones y formularios asociados.
    let operations = 0
    for (const row of (vehicleCheckinsStore.items || []).filter(exactLink)) {
      if (row?.id) {
        await vehicleCheckinsStore.removeItem(row.id)
        operations += 1
      }
    }

    let publicSubmissions = 0
    for (const row of (publicOperationSubmissionsStore.items || []).filter(exactLink)) {
      if (row?.id) {
        await publicOperationSubmissionsStore.removeItem(row.id)
        publicSubmissions += 1
      }
    }
    ...
    if (lead?.id) {
      await leadsStore.removeItem(lead.id)
      leads += 1
    }

    if (moduleName === 'Alojamientos') await removePublicIcalBlock(reservationId)

    return { expenses, tasks, inventoryMovements, operations, publicSubmissions, leads }
  }
  ...
  async function hardDeleteReservation(reservation) {
    ...
    if (!confirm(normalizeStatus(reservation.status) === 'maintenance'
      ? '¿Eliminar este mantenimiento y revertir su impacto en caja, gasto, tareas e inventario?'
      : '¿Eliminar definitivamente esta reserva? Se borrará del calendario y de los registros.')) return
    ...
    setEditingReservation(null)
    showSuccess(normalizeStatus(reservation.status) === 'maintenance'
      ? 'Mantenimiento eliminado. Caja, gasto y tareas vinculadas fueron revertidos.'
      : 'Reserva eliminada definitivamente')
  }
...
${noteLine}`.trim(),
  ...
  // V223.5.16: fuente única para abrir una reserva de alojamiento existente.
  // Normaliza el ID y limpia únicamente el input temporal del próximo abono,
  // conservando intacto el historial de pagos ya registrado.
  function lodgingRecordId(record = {}) { return record.__docId || record.id || record.docId || record.reservationId || record._editingOriginalId || '' }
  function isExistingLodgingDraft(record = {}) { return Boolean(lodgingRecordId(record)) }
  function openEditLodging(reservation = {}) {
    setError('')
    const safeId = lodgingRecordId(reservation)
    setEditingLodging({
      ...reservation,
      id: safeId,
      __docId: reservation.__docId || safeId,
      _editingOriginalId: safeId,
      _originalPaymentAmount: '',
      amount: '',
      amountBs: '',
      amountUsdEquivalent: '',
  ...
  async function syncMaintenanceExpenseMovement(record = {}, moduleName = '', savedId = '') {
    const sourceId = savedId || record.id || record.__docId || ''
    if (!sourceId) return
    const existing = (generalExpensesStore.items || []).find((row) =>
      row.sourceType === 'maintenanceExpense' &&
      String(row.sourceReservationId || '') === String(sourceId) &&
      String(row.sourceModule || '') === String(moduleName || '')
    )
    const isMaintenance = normalizeStatus(record.status) === 'maintenance'
    const costUsd = maintenanceUsdCost(record, exchangeRates)
    const costBs = maintenanceBsCost(record, exchangeRates)
    if (!isMaintenance || costUsd <= 0) {
      if (existing?.id) await generalExpensesStore.removeItem(existing.id)
      return
    }
    const method = maintenanceMovementPaymentMethod(record.maintenancePaymentMethod || 'BS')
    const bucket = paymentBucket(method)
    const assetType = moduleName === 'Renta Car' ? 'Vehículo' : 'Alojamiento'
    const assetId = moduleName === 'Renta Car' ? record.vehicleId : record.accommodationId
    const asset = moduleName === 'Renta Car'
      ? vehicles.find((row) => String(row.id || '') === String(assetId || ''))
      : accommodations.find((row) => String(row.id || '') === String(assetId || ''))
    const payload = {
      date: String(record.startDate || record.createdAt || new Date().toISOString()).slice(0,10),
      transactionType: 'Egreso',
      type: 'Egreso',
      category: 'Mantenimiento',
      description: `${record.maintenanceType || 'Mantenimiento'} · ${asset?.name || 'Activo'}${record.note ? ` · ${record.note}` : ''}`,
      amount: String(costUsd),
      currency: bucket === 'Bs' ? 'Bs' : 'USD',
      amountBs: String(costBs),
      paymentMethod: method,
      expenseStatus: record.expenseStatus || 'Pagado',
      assetType,
      assetId: assetId || '',
      responsible: record.customerName || record.provider || record.createdByName || '',
      bcvDollarRate: String(maintenanceEuroRate(record, exchangeRates) || ''),
      bcvEuroRate: String(maintenanceEuroRate(record, exchangeRates) || ''),
      notes: 'Egreso automático generado desde el formulario de mantenimiento. Usa la misma lógica de caja que Registrar movimiento.',
      sourceReservationId: sourceId,
      sourceType: 'maintenanceExpense',
      sourceModule: moduleName,
      maintenancePaymentMethod: record.maintenancePaymentMethod || '',
      maintenanceCostUsd: costUsd,
      maintenanceCostBs: costBs,
      createdByUid: record.createdByUid || user?.uid || '',
      createdByEmail: record.createdByEmail || user?.email || '',
      createdByName: record.createdByName || sellerName(profile, user),
    }
    if (existing?.id) await generalExpensesStore.editItem(existing.id, { ...existing, ...payload })
    else await generalExpensesStore.createItem(payload)
  }
  ...
  async function saveLodging(event) {
    ...
    const payload = {
      ...
      totalAmountBs: status === 'reserved' ? String(amountBs(total, lodgingRates, lodgingEuroRate)) : '', amountBs: status === 'reserved' ? String(paidBs) : '', amountUsdEquivalent: status === 'reserved' ? String(paidUsd) : '', paymentHistory: status === 'reserved' ? paymentHistory : [],
      maintenanceType: status === 'maintenance' ? editingLodging.maintenanceType || 'Preventivo' : '', maintenanceCost: status === 'maintenance' ? String(maintenanceUsdCost(editingLodging, exchangeRates) || '') : '', maintenanceUsdEquivalent: status === 'maintenance' ? String(maintenanceUsdCost(editingLodging, exchangeRates) || '') : '', maintenancePaymentMethod: status === 'maintenance' ? editingLodging.maintenancePaymentMethod || 'BS' : '', expenseStatus: status === 'maintenance' ? editingLodging.expenseStatus || 'Pagado' : '', bcvDollarRate: status === 'maintenance' ? editingLodging.bcvEuroRate || editingLodging.bcvDollarRate || officialEuroRate || '' : '', bcvEuroRate: status === 'maintenance' ? editingLodging.bcvEuroRate || editingLodging.bcvDollarRate || officialEuroRate || '' : (status === 'reserved' ? (editingLodging.bcvEuroRate || lodgingEuroRate || exchangeRates?.bcvEuro || '') : ''), maintenanceBsCost: status === 'maintenance' ? String(maintenanceBsCost(editingLodging, exchangeRates)) : '', inventoryItemId: status === 'maintenance' ? editingLodging.inventoryItemId || '' : '', inventoryQuantity: status === 'maintenance' ? editingLodging.inventoryQuantity || '' : '',
    ...
    await syncMaintenanceExpenseMovement(savedRecord, 'Alojamientos', savedId)
    ...
    setEditingLodging(status === 'reserved' ? { ...savedRecord, __docId: savedId, _editingOriginalId: savedId, _originalPaymentAmount: '', amount: '', amountBs: '', amountUsdEquivalent: '', paymentReference: '', _paymentProofFile: null, paymentHistory: normalizePaymentHistory(savedRecord) } : savedRecord); setError(''); showSuccess(status === 'maintenance' ? 'Mantenimiento guardado y automatizaciones ERP aplicadas' : 'Reserva guardada, ingreso y tarea de limpieza actualizados')
  }
...
@media print{
  ...
  function reservationFromOperationItem(operationItem = {}) {
    const ids = [
      operationItem?.reservationId,
      operationItem?.id,
      operationItem?.docId,
      operationItem?.__docId,
      operationItem?._docId,
    ].filter(Boolean).map(String)

    const directMatch = reservationsStore.items.find((row) => {
      const rowIds = [
        row?.id,
        row?.__docId,
        row?.docId,
    ...
    const operationDate = String(operationItem?.operationDate || '').slice(0, 10)
    const customer = String(operationItem?.customerName || '').trim().toLowerCase()
    const vehicleId = String(operationItem?.vehicleId || '').trim()

    return reservationsStore.items.find((row) => {
      if (vehicleId && String(row?.vehicleId || '') !== vehicleId) return false
      if (customer && String(row?.customerName || '').trim().toLowerCase() !== customer) return false
      const start = String(row?.startDate || '').slice(0, 10)
      const end = String(row?.endDate || '').slice(0, 10)
      if (!operationDate) return true
...
function printVehicleContractFromOperation(operationItem = {}) {
  ...
  function openVehicleDeliveryForm(operationItem) {
    ...
    if (!vehicle?.id) return setError('Selecciona primero un vehículo.')
    const sourceReservation = reservationFromOperationItem(operationItem) || {}
    const logisticsNote = sourceReservation.note || sourceReservation.notes || operationItem?.logisticsNote || operationItem?.note || operationItem?.notes || ''
    setError('')
    setEditingVehicleDelivery({
      ...
      reservationId: operationItem?.reservationId || sourceReservation?.id || sourceReservation?.__docId || '',
...
async function saveReservation(event = null) {
    ...
    try {
      ...
      const payload = {
        ...
        maintenanceCost: isMaintenance ? String(maintenanceUsdCost(editingReservation, exchangeRates) || '') : '',
        maintenanceUsdEquivalent: isMaintenance ? String(maintenanceUsdCost(editingReservation, exchangeRates) || '') : '',
        ...
        maintenanceLaborCostBs: isMaintenance ? editingReservation.maintenanceLaborCostBs || '' : '',
        maintenancePartsCostBs: isMaintenance ? editingReservation.maintenancePartsCostBs || '' : '',
        ...
        bcvDollarRate: isMaintenance ? editingReservation.bcvEuroRate || editingReservation.bcvDollarRate || officialEuroRate || '' : '',
        bcvEuroRate: isMaintenance ? editingReservation.bcvEuroRate || editingReservation.bcvDollarRate || officialEuroRate || '' : ((isMaintenance || isNoDisponible) ? '' : (editingReservation.bcvEuroRate || reservationEuroRate || exchangeRates?.bcvEuro || '')),
        maintenanceBsCost: isMaintenance ? String(maintenanceBsCost(editingReservation, exchangeRates)) : '',
      ...
      const savedRecord = { ...payload, id: savedId }
      await syncMaintenanceExpenseMovement(savedRecord, 'Renta Car', savedId)
...
@media print{
  ...
  const analytics = useMemo(() => {
    const { start: monthStart, end: monthEnd } = monthBounds(currentMonth)
    const items = reservationsStore.items.filter((record) => {
      if (isIcalImportedBlock(record)) return false
      if (normalizeStatus(record.status) === 'cancelled') return false
      return rangesOverlap(record.startDate, record.endDate, monthStart, monthEnd)
    })
    const vehicleItems = items.filter((record) => String(record.vehicleId || '') === String(selectedVehicle?.id || ''))

    // V223.5.26: el dashboard financiero incluye reservas activas y servicios
    // completados/devueltos. Excluye mantenimiento y bloqueos no comerciales.
    const serviceRows = vehicleItems.filter((record) => {
      const status = normalizeStatus(record.status)
      return status === 'reserved' || status === 'returned'
    })

    const occupied = serviceRows.reduce((sum, record) => {
      const clippedStart = record.startDate < monthStart ? monthStart : record.startDate
      const clippedEnd = record.endDate > monthEnd ? monthEnd : record.endDate
      return sum + dayCount(clippedStart, clippedEnd)
    }, 0)

    const totalService = serviceRows.reduce((sum, record) => {
      const storedTotal = num(record.totalAmount || record.totalUsd || record.quoteTotal || record.serviceTotal)
      const calculatedTotal = typeof totalQuoteForReservation === 'function' ? num(totalQuoteForReservation(record)) : 0
      return sum + (storedTotal || calculatedTotal)
    }, 0)

    const collected = serviceRows.reduce((sum, record) => sum + storedPaidUsd(record, exchangeRates), 0)
    const pending = serviceRows.reduce((sum, record) => sum + pendingAmount(record, exchangeRates), 0)
    const maintenance = vehicleItems.filter((record) => isMaintenanceRecord(record) && !isIcalImportedBlock(record))
    const maintenanceCost = maintenance.reduce((sum, record) => sum + maintenanceUsdCost(record, exchangeRates), 0)
    const cleanings = vehicleItems.filter((record) => record.washStatus === 'completed' || record.cleaningStatus === 'completed' || record.vehicleWashDoneAt || record.receptionStatus === 'completed').length
  ...
  function exportMonthlyExcel() {
    ...
    const rows = monthReservations.map((r) => {
      ...
      return {
        ...
        Tipo_Mantenimiento: r.maintenanceType || '', Costo_Mantenimiento_USD: maintenanceUsdCost(r),
    ...
    const kpis = vehicles.map((vehicle) => {
      ...
      return {
        ...
        Mantenimientos: vehicleRows.filter((r) => isMaintenanceRecord(r) && !isIcalImportedBlock(r)).length,
        Costo_mantenimiento_USD: vehicleRows.filter((r) => isMaintenanceRecord(r) && !isIcalImportedBlock(r)).reduce((sum, r) => sum + maintenanceUsdCost(r), 0),
      }
    })
    const wb = XLSX.utils.book_new()
  ...
  return (
    <main className="app-shell">
      ...
      {(['commercial','quotes','reservations'].includes(moduleMode)) ? <section className="calendar-panel search-module-panel"><div className="topbar"><div><span className="eyebrow">Gestión comercial</span><h2>{searchMode === 'quotes' ? 'Cotizaciones' : 'Reservas'}</h2><p className="vehicle-subtitle">Filtra por nombre, C.I., teléfono o fecha. {isAdmin ? 'Modo admin: puedes editar o eliminar registros.' : 'Solo ves registros creados por tu perfil.'}</p></div><div className="topbar-actions desktop-export-actions"><button className="secondary" onClick={searchMode === 'quotes' ? exportQuotesExcel : exportReservationsExcel}><Download size={16}/> Exportar Excel</button></div></div><div className="global-search-results search-results-full">{globalSearchResults.length ? globalSearchResults.map((row)=><div key={row.id} className="search-result-row"><button type="button" onClick={()=>openGlobalSearchResult(row)}><strong>{row.title}</strong><span>{row.type} · {row.subtitle}</span><small>{row.date} {row.amount ? `· ${money(row.amount)}` : ''}</small></button>{isAdmin && <div className="search-row-actions"><button type="button" className="secondary mini-action" onClick={()=>openGlobalSearchResult(row)}><Pencil size={15}/> Editar</button><button type="button" className="danger mini-action" onClick={()=>deleteSearchResult(row)}><Trash2 size={15}/> Eliminar</button></div>}</div>) : <div className="empty-search">No hay resultados con esos filtros.</div>}</div><button className="secondary mobile-bottom-export" onClick={moduleMode === 'quotes' ? exportQuotesExcel : exportReservationsExcel}><Download size={16}/> Exportar Excel</button></section> : moduleMode === 'hr' ? <section className="calendar-panel hr-panel"><div className="topbar"><div><span className="eyebrow">ERP Base</span><h2>Recursos Humanos</h2><p className="vehicle-subtitle">Personal, roles, tareas operativas, comisiones por vendedor, actividad del equipo y permisos.</p></div><div className="topbar-actions"><button className="secondary" onClick={exportHrExcel}><Download size={16}/> Exportar RRHH</button><button className="secondary" onClick={()=>setEditingHrTask(emptyHrTask())}><Plus size={16}/> Tarea</button><button className="primary" onClick={()=>setEditingHrPerson(emptyHrPerson())}><Plus size={16}/> Personal</button></div></div><section className="analytics-strip hr-kpis"><div><span>Personal activo</span><strong>{hrSummary.activePeople.length}</strong></div><div><span>Tareas pendientes</span><strong>{hrSummary.pendingTasks.length}</strong></div><div><span>Tareas completadas</span><strong>{hrSummary.completedTasks.length}</strong></div><div><span>Vendedores</span><strong>{hrSummary.people.filter((p)=>String(p.role||'').includes('Vendedor')).length}</strong></div><div><span>Comisiones</span><strong>{money(hrSummary.commissionRows.reduce((s,r)=>s+num(r.amount),0))}</strong></div><div><span>Actividad</span><strong>{hrSummary.activityRows.length}</strong></div></section><div className="admin-grid hr-grid"><section className="admin-card"><h3>Personal</h3><p>Registro de colaboradores, fechas laborales, sueldo semanal, tasa $USD BCV y comisión.</p><div className="admin-table"><table><thead><tr><th>Nombre</th><th>Rol</th><th>Acceso</th><th>Ingreso</th><th>Teléfono</th><th>Comisión</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>{hrSummary.people.map((person)=><tr key={person.id}><td>{person.name}</td><td>{person.role}</td><td>{person.appAccess || 'Sin acceso'}</td><td>{person.entryDate ? formatShortDate(person.entryDate) : '-'}</td><td>{person.phone}</td><td>{person.commissionRate}%</td><td>{person.status}</td><td><div className="table-actions"><button className="secondary mini-action" onClick={()=>setEditingHrPerson(person)}><Pencil size={14}/> Editar</button><button className="secondary mini-action" onClick={()=>generateHrPaymentReceipt(person)}><FileText size={14}/> Recibo</button></div></td></tr>)}</tbody></table></div></section><section className="admin-card"><h3>Roles y permisos</h3><p>Matriz base para ordenar qué puede ver y hacer cada perfil.</p><div className="permissions-grid"><div><strong>Admin</strong><span>Ve, edita, elimina, exporta y accede a finanzas/ROI.</span></div><div><strong>Vendedor</strong><span>Crea cotizaciones y reservas. Ve sus propios registros.</span></div><div><strong>Recepción</strong><span>Recibe vehículos, carga km y fotos. No ve finanzas.</span></div><div><strong>Limpieza</strong><span>Ve tareas de alojamientos, marca limpieza y evidencia.</span></div><div><strong>Mantenimiento</strong><span>Gestiona mantenimientos, costos, repuestos y evidencia.</span></div><div><strong>Contabilidad</strong><span>Ve administración, caja, cuentas y exportaciones.</span></div></div></section><section className="admin-card"><h3>Tareas operativas</h3><p>Seguimiento de limpieza, entregas, recepción, mantenimiento y administración.</p><div className="admin-table"><table><thead><tr><th>Tarea</th><th>Responsable</th><th>Fecha</th><th>Prioridad</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>{hrSummary.tasks.map((task)=><tr key={task.id}><td>{task.title}</td><td>{task.responsible}</td><td>{formatShortDate(task.dueDate)}</td><td>{task.priority}</td><td>{task.status}</td><td><div className="table-actions"><button className="secondary mini-action" onClick={()=>setEditingHrTask(task)}><Pencil size={14}/> Editar</button></div></td></tr>)}</tbody></table></div></section><section className="admin-card"><h3>Comisiones por vendedor</h3><p>Resumen base de comisiones generadas desde reservas con pagos registrados.</p><div className="admin-table"><table><thead><tr><th>Vendedor</th><th>Reservas</th><th>Comisión</th><th>Pendiente</th></tr></thead><tbody>{hrSummary.commissionsBySeller.map((row)=><tr key={row.seller}><td>{row.seller}</td><td>{row.reservations}</td><td>{money(row.commission)}</td><td>{money(row.pending)}</td></tr>)}</tbody></table></div></section><section className="admin-card full-span"><h3>Actividad del equipo</h3><p>Registro derivado de reservas, alojamientos, recepción de vehículos e inventario.</p><div className="admin-table"><table><thead><tr><th>Fecha</th><th>Módulo</th><th>Acción</th><th>Usuario</th><th>Detalle</th></tr></thead><tbody>{hrSummary.activityRows.map((row)=><tr key={row.id}><td>{formatShortDate(String(row.date || '').slice(0,10))}</td><td>{row.module}</td><td>{row.action}</td><td>{row.user}</td><td>{row.detail}</td></tr>)}</tbody></table></div></section></div><div className="notice"><AlertCircle size={18}/> Esta versión muestra correctamente RRHH ERP sin alterar reservas, recepción, mantenimiento, ROI, inventario ni administración. Los permisos detallados quedan listos como matriz para endurecer reglas en una siguiente versión.</div></section> : moduleMode === 'inventory' ? <section className="calendar-panel inventory-panel"><div className="topbar"><div><span className="eyebrow">ERP Base</span><h2>Inventario</h2><p className="vehicle-subtitle">Inventario Renta Car, Alojamientos, stock mínimo y movimientos por mantenimiento o limpieza.</p></div><div className="topbar-actions"><button className="secondary" onClick={exportInventoryExcel}><Download size={16}/> Exportar inventario</button><button className="secondary" onClick={()=>setEditingInventoryMovement(emptyInventoryMovement('Entrada'))}><Plus size={16}/> Entrada</button><button className="primary" onClick={()=>setEditingInventoryItem(emptyInventoryItem())}><Plus size={16}/> Artículo</button></div></div><section className="analytics-strip inventory-kpis"><div><span>Artículos</span><strong>{inventorySummary.items.length}</strong></div><div><span>Renta Car</span><strong>{inventorySummary.carItems.length}</strong></div><div><span>Alojamientos</span><strong>{inventorySummary.lodgingItems.length}</strong></div><div><span>Stock mínimo</span><strong>{inventorySummary.lowStock.length}</strong></div><div><span>Valor inventario</span><strong>{money(inventorySummary.totalValue)}</strong></div><div><span>Consumo limpieza</span><strong>{money(inventorySummary.cleaningConsumption)}</strong></div></section><div className="inventory-actions-row"><button className="secondary" onClick={()=>setEditingInventoryMovement({...emptyInventoryMovement('Salida'), reason:'Mantenimiento'})}>Consumo por mantenimiento</button><button className="secondary" onClick={()=>setEditingInventoryMovement({...emptyInventoryMovement('Salida'), reason:'Limpieza', module:'Alojamientos'})}>Consumo por limpieza</button></div>{inventorySummary.lowStock.length > 0 && <div className="notice warning"><AlertCircle size={18}/> Hay {inventorySummary.lowStock.length} artículo(s) en stock mínimo o por debajo del mínimo.</div>}<div className="admin-grid inventory-grid"><section className="admin-card"><h3>Inventario Renta Car</h3><p>Repuestos, aceites, filtros, cauchos, herramientas y productos asociados a vehículos.</p><div className="admin-table"><table><thead><tr><th>Artículo</th><th>Categoría</th><th>Stock</th><th>Mín.</th><th>Costo</th><th>Acciones</th></tr></thead><tbody>{inventorySummary.carItems.map((item)=><tr key={item.id} className={num(item.quantity)<=num(item.minQuantity)?'low-stock-row':''}><td>{item.name}</td><td>{item.category}</td><td>{item.quantity}</td><td>{item.minQuantity}</td><td>{money(item.unitCost)}</td><td><div className="table-actions"><button className="secondary mini-action" onClick={()=>setEditingInventoryItem(item)}><Pencil size={14}/> Editar</button><button className="secondary mini-action" onClick={()=>setEditingInventoryMovement({...emptyInventoryMovement('Salida'), itemId:item.id, module:item.module, assetId:item.assetId, unitCost:item.unitCost, reason:'Mantenimiento'})}>Consumir</button></div></td></tr>)}</tbody></table></div></section><section className="admin-card"><h3>Inventario Alojamientos</h3><p>Lencería, toallas, amenidades, productos de limpieza, llaves, controles y mobiliario.</p><div className="admin-table"><table><thead><tr><th>Artículo</th><th>Categoría</th><th>Stock</th><th>Mín.</th><th>Costo</th><th>Acciones</th></tr></thead><tbody>{inventorySummary.lodgingItems.map((item)=><tr key={item.id} className={num(item.quantity)<=num(item.minQuantity)?'low-stock-row':''}><td>{item.name}</td><td>{item.category}</td><td>{item.quantity}</td><td>{item.minQuantity}</td><td>{money(item.unitCost)}</td><td><div className="table-actions"><button className="secondary mini-action" onClick={()=>setEditingInventoryItem(item)}><Pencil size={14}/> Editar</button><button className="secondary mini-action" onClick={()=>setEditingInventoryMovement({...emptyInventoryMovement('Salida'), itemId:item.id, module:item.module, assetId:item.assetId, unitCost:item.unitCost, reason:'Limpieza'})}>Consumir</button></div></td></tr>)}</tbody></table></div></section><section className="admin-card"><h3>Stock mínimo</h3><p>Artículos que requieren reposición para evitar fallas operativas.</p><div className="admin-table"><table><thead><tr><th>Artículo</th><th>Módulo</th><th>Stock</th><th>Mínimo</th><th>Ubicación</th></tr></thead><tbody>{inventorySummary.lowStock.map((item)=><tr key={item.id}><td>{item.name}</td><td>{item.module}</td><td>{item.quantity}</td><td>{item.minQuantity}</td><td>{inventoryAssetName(item)}</td></tr>)}</tbody></table></div></section><section className="admin-card"><h3>Movimientos de inventario</h3><p>Entradas y salidas registradas, incluyendo consumos por mantenimiento y limpieza.</p><div className="admin-table"><table><thead><tr><th>Fecha</th><th>Tipo</th><th>Motivo</th><th>Artículo</th><th>Cant.</th><th>Total</th></tr></thead><tbody>{inventorySummary.movements.slice(0,16).map((row)=><tr key={row.id}><td>{formatShortDate(row.date)}</td><td>{row.kind}</td><td>{row.reason}</td><td>{row.itemName}</td><td>{row.quantity}</td><td>{money(row.totalCost)}</td></tr>)}</tbody></table></div></section></div><div className="notice"><AlertCircle size={18}/> Esta V82 agrega control de inventario sin alterar reservas, recepción, mantenimiento, ROI ni administración. Los consumos quedan registrados para alimentar egresos y reportes en fases siguientes.</div></section> : moduleMode === 'administration' ? <section className="calendar-panel administration-panel"><div className="topbar"><div><span className="eyebrow">ERP Base</span><h2>Administración</h2><p className="vehicle-subtitle">Caja, ingresos, egresos, cuentas por cobrar, cuentas por pagar, devoluciones y comisiones.</p></div><div className="topbar-actions"><button className="secondary" onClick={()=>openDollarPurchaseForm()}>Compra de $</button><button className="secondary" onClick={openDollarSaleForm}>Venta de $</button><button className="secondary" onClick={exportAdministrationExcel}><Download size={16}/> Exportar administración</button><button className="primary" type="button" onClick={()=>setEditingGeneralExpense(emptyGeneralExpense(profile, user, exchangeRates))}><Plus size={16}/> Registrar movimiento</button></div></div><section className="admin-executive-dashboard"><div className="admin-exec-hero"><span className="eyebrow">Resumen ejecutivo</span><h3>Caja disponible</h3><BsMainAmount usdValue={administrationRows.profit} bsValue={administrationRows.profitBs} exchangeRates={exchangeRates}/><small>Caja principal real en Bs: compras de $ descuentan Bs y ventas de $ suman Bs. No se muestran equivalentes USD falsos en cajas Bs.</small></div><div className="admin-exec-main"><article className="exec-card exec-income"><span>Ingresos cobrados</span><strong><BsMainAmount usdValue={administrationRows.income} bsValue={administrationRows.incomeBs} exchangeRates={exchangeRates}/></strong></article><article className="exec-card exec-expense"><span>Egresos</span><strong><BsMainAmount usdValue={administrationRows.expenses} bsValue={administrationRows.expensesBs} exchangeRates={exchangeRates}/></strong></article><article className="exec-card exec-profit"><span>Utilidad estimada</span><strong><BsMainAmount usdValue={administrationRows.profit} bsValue={administrationRows.profitBs} exchangeRates={exchangeRates}/></strong></article><article className="exec-card exec-receivable"><span>Por cobrar</span><strong><BsMainAmount usdValue={administrationRows.receivableTotal} bsValue={administrationRows.receivableTotalBs} exchangeRates={exchangeRates}/></strong></article></div><div className="admin-alert-row"><article className={administrationRows.payableTotal > 0 ? 'alert-card alert-warning' : 'alert-card'}><span>Por pagar</span><strong><BsMainAmount usdValue={administrationRows.payableTotal} bsValue={administrationRows.payableTotalBs} exchangeRates={exchangeRates}/></strong></article><article className={administrationRows.refundsTotal > 0 ? 'alert-card alert-danger' : 'alert-card'}><span>Devoluciones</span><strong><BsMainAmount usdValue={administrationRows.refundsTotal} exchangeRates={exchangeRates}/></strong></article><article className="alert-card"><span>Comisiones</span><strong><BsMainAmount usdValue={administrationRows.commissionTotal} exchangeRates={exchangeRates}/></strong></article></div><section className="payment-wallet-panel"><div className="wallet-panel-head"><span className="eyebrow">Caja por método de pago</span><strong>Dónde está el dinero</strong></div><div className="wallet-grid">{['Efectivo $','Zelle','Binance'].map((method)=>{ const row = administrationRows.paymentSummary.find((item)=>item.method===method) || {amountUsd:0,amountBs:0,count:0}; const icon = method === 'Efectivo $' ? '💵' : method === 'Zelle' ? '🏦' : '₿'; return <article className={`wallet-card wallet-${method.replace(/[^a-zA-Z0-9]/g,'').toLowerCase()}`} key={method}><div><span className="wallet-icon">{icon}</span><span className="wallet-title">{method}</span></div><strong><WalletAmount method={method} row={row} exchangeRates={exchangeRates}/></strong><small>{row.count} movimientos</small></article> })}</div></section></section><section className="automation-strip"><div><strong>{erpAutomationSummary.incomeAuto}</strong><span>Ingresos automáticos</span></div><div><strong>{erpAutomationSummary.expenseAuto}</strong><span>Egresos mantenimiento</span></div><div><strong>{erpAutomationSummary.inventoryAuto}</strong><span>Consumos inventario</span></div><div><strong>{erpAutomationSummary.cleaningTasks}</strong><span>Tareas limpieza</span></div><div><strong>{erpAutomationSummary.roiUpdates}</strong><span>ROI actualizado</span></div><div><strong>{erpAutomationSummary.commissions}</strong><span>Comisiones auto</span></div></section><div className="admin-grid"><section className="admin-card"><h3>Caja general</h3><p>Movimientos derivados de reservas, abonos, mantenimiento, devoluciones y comisiones.</p><div className="admin-table"><table><thead><tr><th>Fecha</th><th>Tipo</th><th>Categoría</th><th>Módulo</th><th>Monto</th><th>Estado</th></tr></thead><tbody>{administrationRows.cashRows.slice(0,12).map((row)=><tr key={row.id}><td>{formatShortDate(row.date)}</td><td>{row.type}</td><td>{row.category}</td><td>{row.module}</td><td><CashRowAmount row={row} exchangeRates={exchangeRates}/></td><td>{row.status}</td></tr>)}</tbody></table></div></section><section className="admin-card"><h3>Cuentas por cobrar</h3><p>Reservas con saldo pendiente de pago.</p><div className="admin-table"><table><thead><tr><th>Cliente</th><th>Módulo</th><th>Activo</th><th>Pendiente</th><th>Fechas</th><th>Acciones</th></tr></thead><tbody>{administrationRows.receivables.slice(0,12).map((row)=><tr key={row.id}><td>{row.customerName}</td><td>{row.module}</td><td>{row.assetName}</td><td><DualAmount value={row.pendingAmount} exchangeRates={exchangeRates} fallbackRate={row.bcvEuroRate}/></td><td>{formatShortDate(row.startDate)} - {formatShortDate(row.endDate)}</td><td><div className="table-actions"><button className="secondary mini-action" type="button" onClick={()=>openReceivableSource(row)}><Pencil size={14}/> Editar reserva</button><button className="danger mini-action" type="button" onClick={()=>deleteReceivableSource(row)}><Trash2 size={14}/> Eliminar</button></div></td></tr>)}</tbody></table></div></section><section className="admin-card"><h3>Auditoría reciente</h3><p>Últimas acciones críticas registradas por usuario, módulo y fecha.</p><div className="admin-table"><table><thead><tr><th>Fecha</th><th>Acción</th><th>Módulo</th><th>Usuario</th></tr></thead><tbody>{(auditLogsStore?.items || []).slice(0,12).map((row)=><tr key={row.id}><td>{formatShortDate(row.createdAt || row.eventDate)}</td><td>{row.action}</td><td>{row.module}</td><td>{row.userName || row.userEmail || 'Sistema'}</td></tr>)}</tbody></table></div></section><section className="admin-card health-card"><h3>Monitor de salud</h3><p>Estado operativo del sistema, errores recientes y señales de riesgo.</p>{(() => { const snapshot = currentHealthSnapshot(); const recs = healthRecommendations(snapshot); return <><section className="analytics-strip mini-health-strip"><div><span>Estado</span><strong>{snapshot.status.label}</strong></div><div><span>Críticos</span><strong>{snapshot.status.critical}</strong></div><div><span>Altos</span><strong>{snapshot.status.high}</strong></div><div><span>Eventos</span><strong>{snapshot.eventsCount}</strong></div></section><div className="admin-table"><table><thead><tr><th>Fecha</th><th>Severidad</th><th>Módulo</th><th>Mensaje</th></tr></thead><tbody>{healthEvents.slice(0,8).map((row)=><tr key={row.id}><td>{formatShortDate(String(row.createdAt || '').slice(0,10))}</td><td>{row.severity}</td><td>{row.module}</td><td>{row.message}</td></tr>)}{!healthEvents.length && <tr><td colSpan="4">No hay eventos recientes.</td></tr>}</tbody></table></div><ul className="health-recommendations">{recs.map((item, index)=><li key={index}>{item}</li>)}</ul><div className="backup-actions"><button className="secondary" type="button" onClick={exportHealthReportJson}><Download size={16}/> Exportar salud</button><button className="secondary" type="button" onClick={clearHealthEvents}>Limpiar eventos</button></div></> })()}</section><section className="admin-card backup-card"><h3>Backups técnicos</h3><p>Exportación segura de datos operativos con campos sensibles protegidos para auditoría, respaldo y control interno.</p><div className="backup-actions"><button className="secondary" type="button" onClick={exportTechnicalBackupJson}><Download size={16}/> Backup JSON</button><button className="secondary" type="button" onClick={exportTechnicalBackupExcel}><Download size={16}/> Backup Excel</button>{isAdmin && <button className="secondary" type="button" onClick={exportSensitiveAuditJson}><FileText size={16}/> Auditoría sensible</button>}</div><small>Incluye reservas, alojamientos, vehículos, inventario, RRHH, caja derivada, auditoría y operaciones públicas. Se enmascaran datos sensibles.</small></section><section className="admin-card"><h3>Operaciones públicas pendientes</h3><p>Entregas, recepciones y limpiezas enviadas desde links públicos seguros.</p><div className="admin-table"><table><thead><tr><th>Fecha</th><th>Tipo</th><th>Activo</th><th>Acción</th></tr></thead><tbody>{pendingPublicSubmissions.slice(0,12).map((row)=><tr key={row.id}><td>{formatShortDate(row.submittedAt || row.createdAt)}</td><td>{publicSubmissionLabel(row)}</td><td>{publicSubmissionAsset(row)}</td><td><button className="secondary mini-action" onClick={()=>syncPublicOperationSubmission(row)}>Sincronizar</button></td></tr>)}{!pendingPublicSubmissions.length && <tr><td colSpan="4">No hay operaciones públicas pendientes.</td></tr>}</tbody></table></div>{pendingPublicSubmissions.length > 1 && <button className="secondary" onClick={syncAllPublicOperationSubmissions}>Sincronizar pendientes</button>}</section><section className="admin-card"><h3>Movimientos operativos</h3><p>Registra ingresos y gastos administrativos. Los ingresos suman caja; los gastos pagados descuentan caja.</p><div className="admin-table"><table><thead><tr><th>Fecha</th><th>Categoría</th><th>Concepto</th><th>Activo</th><th>Monto</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>{(generalExpensesStore.items || []).slice(0,12).map((row)=><tr key={row.id}><td>{formatShortDate(row.date)}</td><td>{row.category}</td><td>{row.description}</td><td>{row.assetType === 'Vehículo' ? (vehicles.find(v=>v.id===row.assetId)?.name || 'Vehículo') : row.assetType === 'Alojamiento' ? (accommodations.find(a=>a.id===row.assetId)?.name || 'Alojamiento') : 'General'}</td><td><BsMainAmount usdValue={generalExpenseUsdAmount(row, exchangeRates)} bsValue={generalExpenseBsAmount(row, exchangeRates)} exchangeRates={exchangeRates}/></td><td>{row.expenseStatus || 'Pagado'}</td><td><div className="table-actions"><button className="secondary mini-action" type="button" onClick={()=>setEditingGeneralExpense(row)}><Pencil size={14}/> Editar</button><button className="danger mini-action" type="button" onClick={async()=>{ if(confirm('¿Eliminar este gasto?')) { await generalExpensesStore.removeItem(row.id); showSuccess('Gasto eliminado') } }}><Trash2 size={14}/> Borrar</button></div></td></tr>)}{!(generalExpensesStore.items || []).length && <tr><td colSpan="7">No hay movimientos operativos registrados.</td></tr>}</tbody></table></div></section><section className="admin-card"><h3>Cuentas por pagar</h3><p>Solo muestra gastos marcados como Por pagar, devoluciones y comisiones pendientes. Los gastos Pagados ya afectan caja y no aparecen aquí.</p><div className="admin-table"><table><thead><tr><th>Fecha</th><th>Categoría</th><th>Módulo</th><th>Monto</th><th>Responsable</th><th>Acciones</th></tr></thead><tbody>{administrationRows.payables.slice(0,12).map((row)=><tr key={row.id}><td>{formatShortDate(row.date)}</td><td>{row.category}</td><td>{row.module}</td><td><span className="dual-amount"><span className="dual-usd">{payableDisplayAmount(row, exchangeRates)}</span><span className="dual-label">{paymentDisplayMethod(row.method || row.paymentMethod || '')}</span>{String(row.category||'').includes('aliado') || String(row.description||'').toLowerCase().includes('propietario') ? <small>Ganancia Alohandote: {money(row.alohandoteNetIncomeUsd || row.alohandoteIncomeUsd || 0)}</small> : null}</span></td><td>{row.responsible}</td><td><div className="table-actions">{row.category === 'Comisión vendedor' && <button className="primary mini-action" type="button" onClick={()=>openCommissionPayment(row)}>Pagar comisión</button>}<button className="secondary mini-action" type="button" onClick={()=>openPayableSource(row)}><Pencil size={14}/> Editar</button><button className="danger mini-action" type="button" onClick={()=>deletePayableSource(row)}><Trash2 size={14}/> Borrar</button></div></td></tr>)}{!administrationRows.payables.length && <tr><td colSpan="6">No hay cuentas por pagar.</td></tr>}</tbody></table></div></section><section className="admin-card"><h3>Comisiones</h3><p>Cálculo base 15% sobre reservas con pagos registrados. Ajustable en futuras reglas ERP.</p><div className="admin-table"><table><thead><tr><th>Vendedor</th><th>Módulo</th><th>Cliente</th><th>Comisión</th><th>Estado</th></tr></thead><tbody>{administrationRows.commissionRows.slice(0,12).map((row)=><tr key={row.id}><td>{row.responsible || 'Sin vendedor'}</td><td>{row.module}</td><td>{row.description}</td><td>{moneyDual(row.amount, exchangeRates)}</td><td>{row.status}</td></tr>)}</tbody></table></div></section></div><div className="notice"><AlertCircle size={18}/> Esta capa administrativa consolida caja, pagos, inventario, comisiones y operaciones sin alterar el flujo comercial actual. Los movimientos Bs imposibles de cubrir se omiten del saldo principal y quedan en alertas de calidad de datos.</div>{administrationRows.dataQualityRows?.length > 0 && <div className="notice warning"><AlertCircle size={18}/> {administrationRows.dataQualityRows.length} movimiento(s) Bs fueron omitidos del saldo por inconsistencia histórica. Revisa el export de administración para depurar datos viejos.</div>}</section> : moduleMode === 'maintenance' ? <section className="calendar-panel maintenance-panel"><div className="topbar"><div><span className="eyebrow">Operación</span><h2>Mantenimiento</h2><p className="vehicle-subtitle">Dashboard para controlar mantenimientos, costos y próximos servicios por kilometraje.</p></div><div className="topbar-actions"><button className="secondary" type="button" onClick={exportMaintenanceExcel}><Download size={16}/> Exportar mantenimientos</button><button className="primary" type="button" onClick={()=>{ if(!selectedVehicle && vehicles[0]) setSelectedVehicleId(vehicles[0].id); setEditingReservation({ ...emptyReservation((selectedVehicle || vehicles[0])?.id || '', new Date().toISOString().slice(0,10), profile, user), status:'maintenance', currentKm: (selectedVehicle || vehicles[0])?.currentKm || '' })}}>+ Mant. Renta Car</button><button className="secondary" type="button" onClick={()=>{ if(!selectedAccommodation && accommodations[0]) setSelectedAccommodationId(accommodations[0].id); setEditingLodging({ ...emptyLodgingReservation((selectedAccommodation || accommodations[0])?.id || '', new Date().toISOString().slice(0,10), profile, user), status:'maintenance', accommodationName:(selectedAccommodation || accommodations[0])?.name || '', maintenanceType:'Preventivo', endDate:new Date().toISOString().slice(0,10) })}}>+ Mant. alojamiento</button></div></div><section className="analytics-strip"><div><span>Mantenimientos</span><strong>{maintenanceRows.length}</strong></div><div><span>Costo total</span><strong>{money(maintenanceRows.reduce((s,r)=>s+num(r.maintenanceCost),0))}</strong></div><div><span>Próx. por km</span><strong>{upcomingMaintenanceRows.length}</strong></div><div><Mail size={18}/><span>Notificación</span><strong>Email backend</strong></div></section>{upcomingMaintenanceRows.length > 0 && <div className="notice warning"><AlertCircle size={18}/><div><strong>Próximos mantenimientos</strong>{upcomingMaintenanceRows.map((row)=><div key={`alert-${row.id}`}><span>{row.assetName}: {row.remainingKm <= 0 ? `servicio vencido por ${Math.abs(row.remainingKm).toLocaleString('es-VE')} km` : `faltan ${row.remainingKm.toLocaleString('es-VE')} km`} · objetivo {row.targetKm.toLocaleString('es-VE')} km</span></div>)}</div></div>}<div className="profit-table"><table><thead><tr><th>Activo</th><th>Módulo</th><th>Fecha</th><th>Tipo</th><th>Km actual</th><th>Próximo km</th><th>Costo</th><th>Acciones</th></tr></thead><tbody>{maintenanceRows.map((row)=><tr key={`${row.module}-${row.id}`}><td>{row.assetName}</td><td>{row.module}</td><td>{formatShortDate(row.startDate)} - {formatShortDate(row.endDate)}</td><td>{row.maintenanceType || 'Preventivo'}</td><td>{row.currentKm ? row.currentKm.toLocaleString('es-VE') : '-'}</td><td>{row.targetKm ? row.targetKm.toLocaleString('es-VE') : '-'}</td><td>{money(row.maintenanceCost || (num(row.maintenanceLaborCost)+num(row.maintenancePartsCost)))}</td><td><div className="table-actions"><button className="secondary mini-action" onClick={()=>{ if(row.module==='Renta Car'){setSelectedVehicleId(row.vehicleId); setEditingReservation(row)} else {setSelectedAccommodationId(row.accommodationId); setEditingLodging(row)} }}><Pencil size={14}/> Editar</button><button className="danger mini-action" onClick={()=>{ if(row.module==='Renta Car') hardDeleteReservation(row); else hardDeleteLodgingReservation(row) }}><Trash2 size={14}/> Eliminar</button></div></td></tr>)}</tbody></table></div><div className="notice"><Mail size={18}/> Para enviar correos automáticos a jalemanbetanco@gmail.com se requiere Cloud Functions o un backend con SendGrid/Mailgun. Esta V66 deja el dashboard y la estructura lista.</div></section> : moduleMode === 'cars' ? <section className="calendar-panel">
        ...
        {canViewDashboard && (<><section className="analytics-strip"><div><BarChart3 size={18} /><span>Días ocupados</span><strong>{analytics.occupied}</strong></div><div><span>Total servicio</span><strong>{money(analytics.totalService)}</strong></div><div><span>Abonado</span><strong>{money(analytics.collected)}</strong></div><div><span>Pendiente</span><strong>{money(analytics.pending)}</strong></div><button type="button" className="analytics-card-button" onClick={()=>setAssetExpenseDetail({title:`Gastos · ${selectedVehicle?.name || 'Vehículo'}`, rows: analytics.expenseRows})}><span>Gastos</span><strong>{money(analytics.expenseTotal)}</strong></button>{isAlliedVehicle(selectedVehicle) && <div><span>Ganancia aliados</span><strong>{money(analytics.allyProfit)}</strong></div>}</section>{selectedVehicleUpcomingMaintenances.length > 0 && <section className="document-box dashboard-maintenance-detail"><label>Próximo mantenimiento<select value={selectedVehicleUpcomingMaintenances.some((item)=>item.id===selectedVehicleMaintenanceId) ? selectedVehicleMaintenanceId : selectedVehicleUpcomingMaintenances[0]?.id || ''} onChange={(e)=>setSelectedVehicleMaintenanceId(e.target.value)}>{selectedVehicleUpcomingMaintenances.map((item)=>{ const description=String(item.note || item.notes || item.description || item.maintenanceDescription || '').trim(); return <option key={item.id} value={item.id}>{item.maintenanceType || 'Mantenimiento'}{description ? ` · ${description}` : ''} · {item.remainingKm <= 0 ? `Vencido por ${Math.abs(num(item.remainingKm)).toLocaleString('es-VE')} km` : `Faltan ${num(item.remainingKm).toLocaleString('es-VE')} km`} · objetivo {num(item.targetKm).toLocaleString('es-VE')} km</option>})}</select></label>{(() => { const selectedId=selectedVehicleUpcomingMaintenances.some((entry)=>entry.id===selectedVehicleMaintenanceId) ? selectedVehicleMaintenanceId : selectedVehicleUpcomingMaintenances[0]?.id; const item=selectedVehicleUpcomingMaintenances.find((row)=>row.id===selectedId); if(!item) return null; const description=String(item.note || item.notes || item.description || item.maintenanceDescription || '').trim(); return <small><strong>{description || 'Sin observaciones registradas'}</strong> · último servicio: {num(item.serviceKm).toLocaleString('es-VE')} km · kilometraje actual: {num(item.currentKm).toLocaleString('es-VE')} km · próximo servicio: {num(item.targetKm).toLocaleString('es-VE')} km</small> })()}</section>}</>)}
      ...
      {editingReservation && <div className="modal-backdrop"><form className="modal" noValidate onSubmit={saveReservation}><div className="modal-header"><h3>{normalizeStatus(editingReservation.status)==='maintenance' ? 'Registro de mantenimiento' : editingReservationReadOnly ? 'Ver reserva' : editingReservation.id ? 'Editar bloqueo' : 'Nueva reserva'}</h3><button type="button" onClick={() => setEditingReservation(null)}><X size={20} /></button></div>{editingReservation.status !== 'maintenance' && <div className="top-doc-actions">{editingReservation.id && canGenerateDocs(editingReservation) && <button type="button" className="secondary" onClick={() => generateReceipt(editingReservation)}><FileText size={17} /> Recibo PDF</button>}{editingReservation.id && canGenerateDocs(editingReservation) && <button type="button" className="secondary" onClick={() => generateContract(editingReservation)}><FileText size={17} /> Contrato PDF</button>}{(!editingReservationReadOnly || isAdmin || !editingReservation.id || (Boolean(editingReservation?.vehicleId) && ['seller','seller_all','seller_lodging'].includes(normalizeRole(profile?.role)))) && <button type="button" className="secondary" onClick={() => generateQuote(editingReservation)}><FileText size={17} /> Cotizar</button>}</div>}{error && <div className="form-error">{error}</div>}{editingReservation.createdByName && normalizeStatus(editingReservation.status)!=='maintenance' && <div className="notice success"><strong>Reservado por vendedor {editingReservation.createdByName}</strong></div>}
        ...
        {editingReservation.status === 'maintenance' ? <><label>Taller / proveedor<input disabled={editingReservationReadOnly} value={editingReservation.customerName} placeholder="Taller mecánico" onChange={(e) => setEditingReservation({ ...editingReservation, customerName: e.target.value })} /></label><div className="two-columns"><label>Tipo de mantenimiento<select disabled={editingReservationReadOnly} value={editingReservation.maintenanceType || 'Preventivo'} onChange={(e) => setEditingReservation({ ...editingReservation, maintenanceType: e.target.value })}><option>Preventivo</option><option>Correctivo</option></select></label><label>Medio de pago<select disabled={editingReservationReadOnly} value={editingReservation.maintenancePaymentMethod || 'BS'} onChange={(e)=>{ const method=e.target.value; const bucket=maintenancePaymentBucket(method); const rate=maintenanceEuroRate(editingReservation, exchangeRates) || officialEuroRate; const currentUsd=maintenanceUsdCost(editingReservation, exchangeRates); const currentBs=maintenanceBsCost(editingReservation, exchangeRates); setEditingReservation({...editingReservation,maintenancePaymentMethod:method,bcvEuroRate:String(rate||''),bcvDollarRate:String(rate||''),maintenanceBsCost:bucket==='Bs'?String(currentBs || (currentUsd*rate)):String(currentUsd*rate),maintenanceUsdEquivalent:String(currentUsd)}) }}>{MAINTENANCE_PAYMENT_METHODS.map((method)=><option key={method} value={method}>{method}</option>)}</select></label></div>{maintenancePaymentBucket(editingReservation.maintenancePaymentMethod)==='Bs' ? <><div className="two-columns"><label>Costo mano de obra Bs<input disabled={editingReservationReadOnly} type="number" min="0" step="0.01" value={editingReservation.maintenanceLaborCostBs || ''} placeholder="Ej: 15000" onChange={(e)=>{ const rate=maintenanceEuroRate(editingReservation, exchangeRates)||officialEuroRate; const laborBs=num(e.target.value); const partsBs=num(editingReservation.maintenancePartsCostBs); const totalBs=laborBs+partsBs; setEditingReservation({...editingReservation,maintenanceLaborCostBs:e.target.value,maintenanceBsCost:String(totalBs),maintenanceCost:String(rate?Number((totalBs/rate).toFixed(2)):0),maintenanceUsdEquivalent:String(rate?Number((totalBs/rate).toFixed(2)):0),bcvEuroRate:String(rate||''),bcvDollarRate:String(rate||'')}) }}/></label><label>Costo repuestos Bs<input disabled={editingReservationReadOnly} type="number" min="0" step="0.01" value={editingReservation.maintenancePartsCostBs || ''} placeholder="Ej: 25000" onChange={(e)=>{ const rate=maintenanceEuroRate(editingReservation, exchangeRates)||officialEuroRate; const laborBs=num(editingReservation.maintenanceLaborCostBs); const partsBs=num(e.target.value); const totalBs=laborBs+partsBs; setEditingReservation({...editingReservation,maintenancePartsCostBs:e.target.value,maintenanceBsCost:String(totalBs),maintenanceCost:String(rate?Number((totalBs/rate).toFixed(2)):0),maintenanceUsdEquivalent:String(rate?Number((totalBs/rate).toFixed(2)):0),bcvEuroRate:String(rate||''),bcvDollarRate:String(rate||'')}) }}/></label></div></> : <><div className="two-columns"><label>Costo mano de obra $<input disabled={editingReservationReadOnly} type="number" min="0" step="0.01" value={editingReservation.maintenanceLaborCost || ''} placeholder="Ej: 25" onChange={(e)=>{ const total=num(e.target.value)+num(editingReservation.maintenancePartsCost); const rate=maintenanceEuroRate(editingReservation,exchangeRates)||officialEuroRate; setEditingReservation({...editingReservation,maintenanceLaborCost:e.target.value,maintenanceCost:String(total),maintenanceUsdEquivalent:String(total),maintenanceBsCost:String(total*rate),bcvEuroRate:String(rate||''),bcvDollarRate:String(rate||'')}) }}/></label><label>Costo repuesto $<input disabled={editingReservationReadOnly} type="number" min="0" step="0.01" value={editingReservation.maintenancePartsCost || ''} placeholder="Ej: 35" onChange={(e)=>{ const total=num(editingReservation.maintenanceLaborCost)+num(e.target.value); const rate=maintenanceEuroRate(editingReservation,exchangeRates)||officialEuroRate; setEditingReservation({...editingReservation,maintenancePartsCost:e.target.value,maintenanceCost:String(total),maintenanceUsdEquivalent:String(total),maintenanceBsCost:String(total*rate),bcvEuroRate:String(rate||''),bcvDollarRate:String(rate||'')}) }}/></label></div></>}<div className="two-columns"><label>Tasa Euro BCV<input disabled={editingReservationReadOnly} type="number" min="0" step="0.0001" value={editingReservation.bcvEuroRate || editingReservation.bcvDollarRate || officialEuroRate || ''} onChange={(e)=>{ const rate=num(e.target.value); const bs=maintenanceBsCost(editingReservation,exchangeRates); const usd=maintenancePaymentBucket(editingReservation.maintenancePaymentMethod)==='Bs'&&rate?Number((bs/rate).toFixed(2)):maintenanceUsdCost(editingReservation,exchangeRates); setEditingReservation({...editingReservation,bcvEuroRate:e.target.value,bcvDollarRate:e.target.value,maintenanceCost:String(usd),maintenanceUsdEquivalent:String(usd),maintenanceBsCost:String(maintenancePaymentBucket(editingReservation.maintenancePaymentMethod)==='Bs'?bs:Number((usd*rate).toFixed(2)))}) }}/></label><label>Estado de pago<select disabled={editingReservationReadOnly} value={editingReservation.expenseStatus || 'Pagado'} onChange={(e)=>setEditingReservation({...editingReservation, expenseStatus:e.target.value})}>{EXPENSE_PAYMENT_STATUS.map((status)=><option key={status}>{status}</option>)}</select></label></div><div className="two-columns"><div className="pending-box"><span>Costo total del mantenimiento</span><strong>{maintenancePaymentBucket(editingReservation.maintenancePaymentMethod)==='Bs' ? bsMoney(maintenanceBsCost(editingReservation,exchangeRates)) : money(maintenanceUsdCost(editingReservation,exchangeRates))}</strong><small>Equivalente: {money(maintenanceUsdCost(editingReservation,exchangeRates))} / {bsMoney(maintenanceBsCost(editingReservation,exchangeRates))}</small></div><div className="pending-box"><span>Impacto en caja</span><strong>{editingReservation.expenseStatus==='Por pagar'?'No descuenta todavía':`Descuenta Caja ${paymentDisplayMethod(maintenanceMovementPaymentMethod(editingReservation.maintenancePaymentMethod))}`}</strong><small>Se registrará automáticamente como gasto del vehículo.</small></div></div><label className="file-pick">Facturas / fotos del mantenimiento<input disabled={editingReservationReadOnly} type="file" multiple accept="image/jpeg,image/png,image/webp,image/heic,image/heif,.jpg,.jpeg,.png,.webp,.heic,.heif,.pdf" onChange={(e)=>setEditingReservation({...editingReservation,_maintenanceInvoiceFiles:Array.from(e.target.files || [])})}/><small>{editingReservation._maintenanceInvoiceFiles?.length ? `${editingReservation._maintenanceInvoiceFiles.length} archivo(s) seleccionado(s)` : editingReservation.maintenanceInvoices?.length ? `${editingReservation.maintenanceInvoices.length} factura(s) cargada(s)` : 'Puedes cargar fotos o imágenes de facturas'}</small></label><div className="two-columns"><label>Inventario usado<select disabled={editingReservationReadOnly} value={editingReservation.inventoryItemId || ''} onChange={(e)=>{ const item = inventoryItemsStore.items.find((row)=>row.id===e.target.value) || {}; const partsCost = editingReservation.maintenancePartsCost || (item.unitCost || ''); const total = num(editingReservation.maintenanceLaborCost)+num(partsCost); setEditingReservation({...editingReservation, inventoryItemId:e.target.value, inventoryQuantity: editingReservation.inventoryQuantity || (e.target.value ? '1' : ''), maintenancePartsCost: partsCost, maintenanceCost: editingReservation.maintenanceCost || String(total), maintenanceBsCost: String(maintenanceBsCost({maintenanceCost: editingReservation.maintenanceCost || total, bcvDollarRate: editingReservation.bcvDollarRate}, exchangeRates)) }) }}><option value="">Sin consumo automático</option>{inventoryItemsStore.items.filter((item)=>item.module==='Renta Car').map((item)=><option key={item.id} value={item.id}>{item.name} · Stock {item.quantity}</option>)}</select></label><label>Cantidad inventario<input disabled={editingReservationReadOnly || !editingReservation.inventoryItemId} type="number" min="0" value={editingReservation.inventoryQuantity || ''} onChange={(e)=>setEditingReservation({...editingReservation,inventoryQuantity:e.target.value})}/></label></div><div className="two-columns"><label>KM actual<input disabled={editingReservationReadOnly} type="number" value={editingReservation.currentKm || selectedVehicle?.currentKm || ''} onChange={(e)=>setEditingReservation({...editingReservation,currentKm:e.target.value,maintenanceTargetKm:String(num(e.target.value)+num(editingReservation.nextMaintenanceEveryKm||0))})}/></label><label>Intervalo para próximo mantenimiento (km)<input disabled={editingReservationReadOnly} type="number" value={editingReservation.nextMaintenanceEveryKm||''} placeholder="Ej: 5000" onChange={(e)=>setEditingReservation({...editingReservation,nextMaintenanceEveryKm:e.target.value,maintenanceTargetKm:String(num(editingReservation.currentKm || selectedVehicle?.currentKm || 0)+num(e.target.value))})}/></label></div><label>KM objetivo de servicio<input disabled type="number" value={editingReservation.maintenanceTargetKm||''} readOnly /></label><small className="muted">La alerta aparecerá únicamente cuando falten 200 km o menos para este servicio.</small><label>Observación / descripción<textarea disabled={editingReservationReadOnly} rows="4" value={editingReservation.note} onChange={(e) => setEditingReservation({ ...editingReservation, note: e.target.value })} /></label></> : <>
          ...
          <div className="two-columns"><label>Costo total del servicio $<input disabled={editingReservationReadOnly} type="number" value={editingReservation.totalAmount || ''} placeholder="Ej: 200" onChange={(e) => handleTotalAmountChange(e.target.value)} /></label></div>{(() => { const existingReservation = Boolean(editingReservation.id || editingReservation.__docId || editingReservation._editingOriginalId); const reservationPending = frozenPendingUsdForDisplay(editingReservation, editingReservation.totalAmount || totalQuoteForReservation(editingReservation), rateAwareExchangeRates, editingReservation.bcvEuroRate || officialEuroRate); const paymentFields = <div className="two-columns"><label>{existingReservation ? (isBsPaymentMethod(editingReservation.paymentMethod) ? 'Nuevo abono Bs' : 'Nuevo abono $') : paymentInputLabel(editingReservation.paymentMethod)}<input disabled={editingReservationReadOnly} type="number" min="0" step="0.01" value={editingReservation.amount || ''} placeholder={existingReservation ? 'Ingresa únicamente el nuevo abono' : paymentInputPlaceholder(editingReservation.paymentMethod)} onChange={(e) => setEditingReservation({ ...editingReservation, amount: e.target.value, amountBs: String(paymentAmountBs(e.target.value, editingReservation.paymentMethod, rateAwareExchangeRates, editingReservation.bcvEuroRate || officialEuroRate)), amountUsdEquivalent: String(paymentAmountUsd(e.target.value, editingReservation.paymentMethod, rateAwareExchangeRates, editingReservation.bcvEuroRate || officialEuroRate)) })} />{existingReservation && <small>El historial anterior se conserva. Este campo registra solamente el nuevo abono.</small>}</label><label>Método del abono<select disabled={editingReservationReadOnly} value={editingReservation.paymentMethod || '$ Efectivo'} onChange={(e)=>setEditingReservation({...editingReservation,paymentMethod:e.target.value,amountBs:String(paymentAmountBs(editingReservation.amount || '', e.target.value, rateAwareExchangeRates, editingReservation.bcvEuroRate || officialEuroRate)),amountUsdEquivalent:String(paymentAmountUsd(editingReservation.amount || '', e.target.value, rateAwareExchangeRates, editingReservation.bcvEuroRate || officialEuroRate))})}>{PAYMENT_METHODS.map((method)=><option key={method}>{method}</option>)}</select></label></div>; return existingReservation && reservationPending > 0.01 ? <section className="document-box rentcar-new-payment-box"><h4>Registrar abono adicional</h4><div className="notice"><strong>Saldo pendiente: {money(reservationPending)}</strong><span>Agrega aquí el nuevo pago sin modificar los abonos anteriores.</span></div>{paymentFields}</section> : paymentFields })()}<div className="two-columns"><div className="pending-box"><span>Abono equivalente $</span><strong>{money(frozenPaidUsdForDisplay(editingReservation, rateAwareExchangeRates, editingReservation.bcvEuroRate || officialEuroRate))}</strong><small>{bsMoney(frozenPaidBsForDisplay(editingReservation, rateAwareExchangeRates, editingReservation.bcvEuroRate || officialEuroRate))}</small></div></div>
          ...
          <label>Instrucciones logísticas<textarea disabled={editingReservationReadOnly} rows="4" value={editingReservation.note} placeholder="Entrega, depósito, condiciones, dirección, hora de entrega, etc." onChange={(e) => setEditingReservation({ ...editingReservation, note: e.target.value })} /></label>
        </>}
      ...
      {editingLodging && <div className="modal-backdrop"><form className="modal" onSubmit={saveLodging}><div className="modal-header"><h3>{normalizeStatus(editingLodging.status)==='maintenance' ? (isExistingLodgingDraft(editingLodging) ? 'Editar ticket de mantenimiento' : 'Nuevo Ticket de mantenimiento') : (isExistingLodgingDraft(editingLodging) ? 'Editar reserva alojamiento' : 'Nueva reserva alojamiento')}</h3><button type="button" onClick={() => setEditingLodging(null)}><X size={20} /></button></div>{editingLodging.status !== 'maintenance' && <div className="top-doc-actions">{isExistingLodgingDraft(editingLodging) && normalizeStatus(editingLodging.status)==='reserved' && <button type="button" className="secondary" onClick={() => generateLodgingReceipt(editingLodging)}><FileText size={17}/> Recibo PDF</button>}{isExistingLodgingDraft(editingLodging) && normalizeStatus(editingLodging.status)==='reserved' && <button type="button" className="secondary" onClick={() => generateLodgingQuote(editingLodging)}><FileText size={17}/> Cotizar</button>}</div>}{error && <div className="form-error">{error}</div>}<label>Alojamiento<select value={editingLodging.accommodationId} onChange={(e)=>{const apt=accommodations.find(a=>a.id===e.target.value); const baseTotal=lodgingTotal(lodgingNights(editingLodging.startDate, editingLodging.endDate), apt?.nightlyRate||'', apt?.cleaningFee||''); const allyIncome=isAlliedAccommodation(apt)?allyIncomeTargetUsd(baseTotal, apt?.allyProfitMode||'fixed', apt?.allyProfitValue||0):''; const next={...editingLodging, accommodationId:e.target.value, nightlyRate:apt?.nightlyRate||'', cleaningFee:apt?.cleaningFee||'', lodgingOwnershipType:isAlliedAccommodation(apt)?'Aliado':'Propio', allyProfitMode:apt?.allyProfitMode||'fixed', allyProfitValue:apt?.allyProfitValue||'', alohandoteIncomeUsd:allyIncome, ownerShareUsd:isAlliedAccommodation(apt)?Math.max(0, Number((baseTotal-num(allyIncome)).toFixed(2))):''}; updateLodgingDates(next)}}>{accommodations.map((apt)=><option key={apt.id} value={apt.id}>{apt.name}</option>)}</select></label><div className="two-columns"><label>{normalizeStatus(editingLodging.status)==='maintenance' ? 'Fecha de inicio' : 'Check in'}<input type="date" value={editingLodging.startDate} onChange={(e)=>updateLodgingDates({...editingLodging,startDate:e.target.value})}/></label><label>{normalizeStatus(editingLodging.status)==='maintenance' ? 'Fecha de término' : 'Check out'}<input type="date" value={editingLodging.endDate} onChange={(e)=>updateLodgingDates({...editingLodging,endDate:e.target.value})}/></label></div><label>Estado<select value={editingLodging.status} onChange={(e)=>setEditingLodging({...editingLodging,status:e.target.value})}>{reservationStatusOptions.map(([key,value])=><option key={key} value={key}>{value.label}</option>)}</select></label>{editingLodging.status === 'maintenance' ? <><label>Proveedor / motivo<input value={editingLodging.customerName||''} onChange={(e)=>setEditingLodging({...editingLodging,customerName:e.target.value})}/></label><div className="two-columns"><label>Tipo de mantenimiento<select value={editingLodging.maintenanceType||'Preventivo'} onChange={(e)=>setEditingLodging({...editingLodging,maintenanceType:e.target.value})}><option>Preventivo</option><option>Correctivo</option></select></label><label>Medio de pago<select value={editingLodging.maintenancePaymentMethod || 'BS'} onChange={(e)=>{ const method=e.target.value; const rate=maintenanceEuroRate(editingLodging,exchangeRates)||officialEuroRate; const usd=maintenanceUsdCost(editingLodging,exchangeRates); const bs=maintenanceBsCost(editingLodging,exchangeRates); setEditingLodging({...editingLodging,maintenancePaymentMethod:method,bcvEuroRate:String(rate||''),bcvDollarRate:String(rate||''),maintenanceCost:String(usd),maintenanceUsdEquivalent:String(usd),maintenanceBsCost:String(bs||usd*rate)}) }}>{MAINTENANCE_PAYMENT_METHODS.map((method)=><option key={method} value={method}>{method}</option>)}</select></label></div><div className="two-columns"><label>{maintenancePaymentBucket(editingLodging.maintenancePaymentMethod)==='Bs'?'Costo mantenimiento Bs':'Costo mantenimiento $'}<input type="number" min="0" step="0.01" value={maintenancePaymentBucket(editingLodging.maintenancePaymentMethod)==='Bs'?(editingLodging.maintenanceBsCost||''):(editingLodging.maintenanceCost||'')} onChange={(e)=>{ const rate=maintenanceEuroRate(editingLodging,exchangeRates)||officialEuroRate; if(maintenancePaymentBucket(editingLodging.maintenancePaymentMethod)==='Bs'){ const bs=num(e.target.value); const usd=rate?Number((bs/rate).toFixed(2)):0; setEditingLodging({...editingLodging,maintenanceBsCost:e.target.value,maintenanceCost:String(usd),maintenanceUsdEquivalent:String(usd),bcvEuroRate:String(rate||''),bcvDollarRate:String(rate||'')}) } else { const usd=num(e.target.value); setEditingLodging({...editingLodging,maintenanceCost:e.target.value,maintenanceUsdEquivalent:e.target.value,maintenanceBsCost:String(Number((usd*rate).toFixed(2))),bcvEuroRate:String(rate||''),bcvDollarRate:String(rate||'')}) } }}/></label><label>Tasa Euro BCV<input type="number" min="0" step="0.0001" value={editingLodging.bcvEuroRate || editingLodging.bcvDollarRate || officialEuroRate || ''} onChange={(e)=>{ const rate=num(e.target.value); const bs=maintenanceBsCost(editingLodging,exchangeRates); const usd=maintenancePaymentBucket(editingLodging.maintenancePaymentMethod)==='Bs'&&rate?Number((bs/rate).toFixed(2)):maintenanceUsdCost(editingLodging,exchangeRates); setEditingLodging({...editingLodging,bcvEuroRate:e.target.value,bcvDollarRate:e.target.value,maintenanceCost:String(usd),maintenanceUsdEquivalent:String(usd),maintenanceBsCost:String(maintenancePaymentBucket(editingLodging.maintenancePaymentMethod)==='Bs'?bs:Number((usd*rate).toFixed(2)))}) }}/></label></div><div className="two-columns"><label>Estado de pago<select value={editingLodging.expenseStatus || 'Pagado'} onChange={(e)=>setEditingLodging({...editingLodging, expenseStatus:e.target.value})}>{EXPENSE_PAYMENT_STATUS.map((status)=><option key={status}>{status}</option>)}</select></label><div className="pending-box"><span>Costo total / equivalente</span><strong>{maintenancePaymentBucket(editingLodging.maintenancePaymentMethod)==='Bs'?bsMoney(maintenanceBsCost(editingLodging,exchangeRates)):money(maintenanceUsdCost(editingLodging,exchangeRates))}</strong><small>{money(maintenanceUsdCost(editingLodging,exchangeRates))} / {bsMoney(maintenanceBsCost(editingLodging,exchangeRates))}</small></div></div><div className="pending-box"><span>Impacto en caja</span><strong>{editingLodging.expenseStatus==='Por pagar'?'No descuenta todavía':`Descuenta Caja ${paymentDisplayMethod(maintenanceMovementPaymentMethod(editingLodging.maintenancePaymentMethod))}`}</strong><small>Se registrará automáticamente como gasto del alojamiento.</small></div><div className="two-columns"><label>Inventario usado<select value={editingLodging.inventoryItemId || ''} onChange={(e)=>{ const item = inventoryItemsStore.items.find((row)=>row.id===e.target.value) || {}; setEditingLodging({...editingLodging, inventoryItemId:e.target.value, inventoryQuantity: editingLodging.inventoryQuantity || (e.target.value ? '1' : ''), maintenanceCost: editingLodging.maintenanceCost || (item.unitCost || '') }) }}><option value="">Sin consumo automático</option>{inventoryItemsStore.items.filter((item)=>item.module==='Alojamientos').map((item)=><option key={item.id} value={item.id}>{item.name} · Stock {item.quantity}</option>)}</select></label><label>Cantidad inventario<input disabled={!editingLodging.inventoryItemId} type="number" min="0" value={editingLodging.inventoryQuantity || ''} onChange={(e)=>setEditingLodging({...editingLodging,inventoryQuantity:e.target.value})}/></label></div><label>Observación / detalle del mantenimiento<textarea rows="4" value={editingLodging.note||''} placeholder="Describe la reparación, repuestos, proveedor, diagnóstico o trabajo realizado..." onChange={(e)=>setEditingLodging({...editingLodging,note:e.target.value})}/></label></> : editingLodging.status === 'pending' ? <label>Observación<textarea rows="4" value={editingLodging.note||''} placeholder="Motivo del bloqueo" onChange={(e)=>setEditingLodging({...editingLodging,note:e.target.value})}/></label> : <><label>Vendedor<select disabled={!isAdmin && isSellerProfile(profile?.role)} value={editingLodging.createdByName || normalizePersonName(sellerName(profile, user))} onChange={(e)=>setEditingLodging({...editingLodging,createdByName:e.target.value})}>{sellerOptionsForModule('lodging').map((name)=><option key={name} value={name}>{name}</option>)}</select></label><label>Huésped<input value={editingLodging.customerName||''} placeholder="Nombre del huésped" onBlur={()=>applyLeadToLodgingDraft(editingLodging)} onChange={(e)=>setEditingLodging({...editingLodging,customerName:e.target.value})}/></label><div className="two-columns"><label>Cédula / identificación<div className="id-inline"><select value={editingLodging.customerIdType || (String(editingLodging.customerId||'').toUpperCase().startsWith('E-') ? 'E' : 'V')} onChange={(e)=>setEditingLodging({...editingLodging,customerIdType:e.target.value,customerNationality:e.target.value==='E'?'extranjero':'venezolano'})}><option value="V">V</option><option value="E">E</option></select><input value={editingLodging.customerId||''} placeholder="Ej: 12.345.678" onBlur={()=>applyLeadToLodgingDraft(editingLodging)} onChange={(e)=>setEditingLodging({...editingLodging,customerId:e.target.value})}/></div></label><label>Teléfono<input value={editingLodging.phone||''} placeholder="WhatsApp" onBlur={()=>applyLeadToLodgingDraft(editingLodging)} onChange={(e)=>setEditingLodging({...editingLodging,phone:e.target.value})}/></label></div><label>Correo electrónico<input type="email" value={editingLodging.email||''} placeholder="correo del huésped" onChange={(e)=>setEditingLodging({...editingLodging,email:e.target.value})}/></label><div className="two-columns"><label>Canal<select value={editingLodging.channel||'Cliente nuevo'} onChange={(e)=>setEditingLodging({...editingLodging,channel:e.target.value})}>{LODGING_CHANNELS.map((ch)=><option key={ch}>{ch}</option>)}</select></label></div><section className="quote-box"><h4>Cálculo de alojamiento</h4><div className="rate-box compact-rates"><div><span>Euro</span><strong>{ratesLoading && !exchangeRates ? 'Consultando...' : (exchangeRates?.bcvEuro || editingLodging.bcvEuroRate) ? `${Number(exchangeRates?.bcvEuro || editingLodging.bcvEuroRate).toFixed(2)} Bs` : 'No disponible'}</strong></div><div className="rate-highlight wide-rate"><span>Costo en BS</span><strong>{(editingLodging.bcvEuroRate || officialEuroRate || exchangeRates?.bcvEuro) ? bsMoney(amountBs(editingLodging.totalAmount || lodgingTotal(lodgingNights(editingLodging.startDate, editingLodging.endDate), editingLodging.nightlyRate, editingLodging.cleaningFee), rateAwareExchangeRates, editingLodging.bcvEuroRate || officialEuroRate)) : 'No disponible'}</strong></div></div><div className="two-columns"><label>Número de noches<input type="number" readOnly value={lodgingNights(editingLodging.startDate, editingLodging.endDate)}/></label><label>Costo por noche $<input type="number" value={editingLodging.nightlyRate||''} onChange={(e)=>handleLodgingRateChange(e.target.value)}/></label></div><div className="two-columns"><label>Costo de limpieza $<input type="number" value={editingLodging.cleaningFee||''} onChange={(e)=>handleLodgingCleaningChange(e.target.value)}/></label>{isAdmin ? <label>Total alojamiento $<input type="number" value={editingLodging.totalAmount || ''} onChange={(e)=>setEditingLodging({...editingLodging,totalAmount:e.target.value})}/></label> : <div className="pending-box"><span>Total alojamiento</span><strong>{money(editingLodging.totalAmount || lodgingTotal(lodgingNights(editingLodging.startDate, editingLodging.endDate), editingLodging.nightlyRate, editingLodging.cleaningFee))}</strong></div>}</div>{!isAdmin && <div className="pending-box"><span>Comisión vendedor 15%</span><strong>{money(commissionFromTotal(editingLodging.totalAmount || lodgingTotal(lodgingNights(editingLodging.startDate, editingLodging.endDate), editingLodging.nightlyRate, editingLodging.cleaningFee)))}</strong></div>}</section>{(() => { const existingLodging = isExistingLodgingDraft(editingLodging); const lodgingCurrentTotal = editingLodging.totalAmount || lodgingTotal(lodgingNights(editingLodging.startDate, editingLodging.endDate), editingLodging.nightlyRate, editingLodging.cleaningFee); const lodgingPending = frozenPendingUsdForDisplay(editingLodging, lodgingCurrentTotal, rateAwareExchangeRates, editingLodging.bcvEuroRate || officialEuroRate); const paymentFields = <div className="two-columns"><label>{existingLodging ? (isBsPaymentMethod(editingLodging.paymentMethod) ? 'Nuevo abono Bs' : 'Nuevo abono $') : paymentInputLabel(editingLodging.paymentMethod)}<input type="number" min="0" step="0.01" value={editingLodging.amount || ''} placeholder={existingLodging ? 'Ingresa únicamente el nuevo abono' : paymentInputPlaceholder(editingLodging.paymentMethod)} onChange={(e)=>setEditingLodging({...editingLodging,amount:e.target.value,amountBs:String(paymentAmountBs(e.target.value, editingLodging.paymentMethod, rateAwareExchangeRates, editingLodging.bcvEuroRate || officialEuroRate)),amountUsdEquivalent:String(paymentAmountUsd(e.target.value, editingLodging.paymentMethod, rateAwareExchangeRates, editingLodging.bcvEuroRate || officialEuroRate))})}/>{existingLodging && <small>El historial anterior se conserva. Este campo registra solamente el nuevo abono.</small>}</label><label>Método del abono<select value={editingLodging.paymentMethod || '$ Efectivo'} onChange={(e)=>setEditingLodging({...editingLodging,paymentMethod:e.target.value,amountBs:String(paymentAmountBs(editingLodging.amount || '', e.target.value, rateAwareExchangeRates, editingLodging.bcvEuroRate || officialEuroRate)),amountUsdEquivalent:String(paymentAmountUsd(editingLodging.amount || '', e.target.value, rateAwareExchangeRates, editingLodging.bcvEuroRate || officialEuroRate))})}>{PAYMENT_METHODS.map((method)=><option key={method}>{method}</option>)}</select></label></div>; return existingLodging && lodgingPending > 0.01 ? <section className="document-box lodging-new-payment-box"><h4>Registrar abono adicional</h4><div className="notice"><strong>Saldo pendiente: {money(lodgingPending)}</strong><span>Agrega aquí el nuevo pago sin modificar los abonos anteriores.</span></div>{paymentFields}</section> : paymentFields })()}<div className="two-columns"><div className="pending-box"><span>Abono equivalente $</span><strong>{money(frozenPaidUsdForDisplay(editingLodging, rateAwareExchangeRates, editingLodging.bcvEuroRate || officialEuroRate))}</strong><small>{bsMoney(frozenPaidBsForDisplay(editingLodging, rateAwareExchangeRates, editingLodging.bcvEuroRate || officialEuroRate))}</small></div><div className="pending-box"><span>Diferencia a pagar</span><strong>{moneyDual(frozenPendingUsdForDisplay(editingLodging, editingLodging.totalAmount || lodgingTotal(lodgingNights(editingLodging.startDate, editingLodging.endDate), editingLodging.nightlyRate, editingLodging.cleaningFee), rateAwareExchangeRates, editingLodging.bcvEuroRate || officialEuroRate), rateAwareExchangeRates, editingLodging.bcvEuroRate || officialEuroRate)}</strong></div></div><section className="document-box"><h4>Pago / comprobante</h4><div className="two-columns"><label>Método registrado<input value={editingLodging.paymentMethod || '$ Efectivo'} readOnly /></label><label>Nº de Referencia<input value={editingLodging.paymentReference || ''} placeholder="Referencia del pago" onChange={(e)=>setEditingLodging({...editingLodging,paymentReference:e.target.value})}/></label></div><input type="file" accept=".pdf,.jpg,.jpeg,.png,.webp" onChange={(e)=>setEditingLodging({...editingLodging,_paymentProofFile:e.target.files?.[0] || null})}/><small>{editingLodging._paymentProofFile ? `Comprobante seleccionado: ${editingLodging._paymentProofFile.name}` : editingLodging.paymentProof?.url ? <a href={editingLodging.paymentProof.url} target="_blank">Ver comprobante cargado</a> : 'Comprobante pendiente'}</small></section>{renderPaymentHistoryManager(editingLodging, setEditingLodging, false)}<label>Observación<textarea rows="4" value={editingLodging.note||''} placeholder="Condiciones, dirección, datos de llegada, etc." onChange={(e)=>setEditingLodging({...editingLodging,note:e.target.value})}/></label></>}<div className="modal-actions bottom-actions">{isExistingLodgingDraft(editingLodging) && <button type="button" className="danger" onClick={()=>hardDeleteLodgingReservation(editingLodging)}><Trash2 size={17}/> Eliminar</button>}{isExistingLodgingDraft(editingLodging) && normalizeStatus(editingLodging.status)==="reserved" && storedPaidBs(editingLodging, exchangeRates)>0 && <button type="button" className="secondary" onClick={()=>openRefundLodging(editingLodging)}><Download size={17}/> Anulación/Devolución</button>}{!isExistingLodgingDraft(editingLodging) && normalizeStatus(editingLodging.status)==='reserved' && <button type="button" className="secondary" onClick={() => generateLodgingQuote(editingLodging)}><FileText size={17}/> Cotizar</button>}<button type="submit" className="primary">Guardar</button></div></form></div>}
